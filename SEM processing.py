...
def process_sem_image(image_path, threshold_method='manual', target_size=(1024, 1024)):
    ...
    # cv2.imshow("155##.Grayscale image after filling", resized_img)
    # cv2.imwrite(os.path.join(save_folder, "155##.Grayscale_image_after_filling.png"), resized_img)
    saved_image_path = os.path.join(save_folder, "155##.Grayscale_image_after_filling.png")
   ...
   # cv2.imwrite(os.path.join(save_folder, "3_NO.After_enhancing_the_border.png"), enhanced_img)
    ...
    # cv2.imshow("158#.After binarization", binary_img)
    #cv2.imwrite(os.path.join(save_folder, "158#.After_binarization.png"), binary_img)
   ...
   # cv2.imwrite(os.path.join(save_folder, "6#.After_removing_the_weak_noise.png"), binary_img)
    ...
    final_img = remove_high_irregularity_areas(binary_img, contours, irregularities, threshold=20)
...
if __name__ == "__main__":
    ...
    for folder_idx in range(1, 158):
        # 全局孔隙计数器
        GLOBAL_PORE_COUNTER = 0  # 初始化计数器

        image_dir = f"../Pore_Classification/SEM/{folder_idx}"
